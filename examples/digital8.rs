...
fn create_scene(mut cmd: Commands, asset_server: Res<AssetServer>) {
    ...
    // Spawn Virtual Joystick with 8-direction digital snapping
    create_joystick(
        &mut cmd,
...
fn update_joystick(
    mut joystick: MessageReader<VirtualJoystickMessage<String>>,
    ...
    for j in joystick.read() {
        let Vec2 { x, y } = j.axis();
        // With JoystickDigital8, the delta will always be one of 8 directions
        // (N, NE, E, SE, S, SW, W, NW) or zero, normalized to unit length
